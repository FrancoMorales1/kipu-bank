...
contract KipuBank {
    ...
    */
    ...
    modifier withinBankCap(uint256 _amount) {
        if (address(this).balance + _amount > s_bankCap) {
    ...
    /// @notice Prevent direct transfers so users must use {deposit}.
    receive() external payable {
        revert DirectTransferAttempt();
    }
